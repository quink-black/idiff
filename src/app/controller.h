...
public:
    ...
    // Overlay, full Split grid) survive the switch.  The group's
    // comparison reference claims one of those slots: the config
    // convention seeds it with the item titled "原图" on first
    // visit, and an explicit mark-as-reference (GUI or RPC) takes
    // precedence and survives group navigation.  Status messages from
    // the service are forwarded through the status reporter.
