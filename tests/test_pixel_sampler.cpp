...
// -- sample_image: empty / out-of-range ---------------------------------

TEST_CASE("sample_image: empty matrix returns invalid", "[pixel_sampler]") {
    cv::Mat empty;
    auto s = sample_image(empty, 0.5, 0.5);
    REQUIRE_FALSE(s.valid);
}

TEST_CASE("sample_image: u >= 1.0 is out of range", "[pixel_sampler]") {
...
TEST_CASE("sample_image: negative u is out of range", "[pixel_sampler]") {
    cv::Mat m(4, 4, CV_8UC3, cv::Scalar(10, 20, 30));
    REQUIRE_FALSE(sample_image(m, -0.1, 0.5).valid);
    REQUIRE_FALSE(sample_image(m, 0.5, -0.1).valid);
}

// -- sample_image: depth coverage ---------------------------------------

TEST_CASE("sample_image: CV_8UC1 picks correct pixel", "[pixel_sampler]") {
...
TEST_CASE("sample_image: CV_8UC3 picks correct pixel", "[pixel_sampler]") {
...
TEST_CASE("sample_image: CV_8UC4 picks correct pixel", "[pixel_sampler]") {
...
TEST_CASE("sample_image: CV_16UC3 carries full 16-bit range", "[pixel_sampler]") {
...
TEST_CASE("sample_image: CV_32FC3 preserves float values", "[pixel_sampler]") {
...
// -- pixel_to_norm / norm_to_pixel round trip ----------------------------

TEST_CASE("pixel_to_norm: pixel center convention", "[pixel_sampler]") {
    REQUIRE(pixel_to_norm(0, 4) == Catch::Approx(0.125));
    REQUIRE(pixel_to_norm(3, 4) == Catch::Approx(0.875));
}

TEST_CASE("pixel_to_norm: w == 0 returns 0.5 instead of dividing", "[pixel_sampler]") {
    REQUIRE(pixel_to_norm(0, 0) == Catch::Approx(0.5));
}

TEST_CASE("norm_to_pixel: 0.5 falls in middle", "[pixel_sampler]") {
    REQUIRE(norm_to_pixel(0.5, 4) == 2);
    REQUIRE(norm_to_pixel(0.0, 4) == 0);
    REQUIRE(norm_to_pixel(0.999, 4) == 3);
}

TEST_CASE("norm_to_pixel/pixel_to_norm round trip preserves index",
          "[pixel_sampler]") {
